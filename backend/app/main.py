...
from app.routers import paperless, correspondents, tags, document_types, settings, llm, debug, statistics, ignored_items, ocr, cleanup, classifier, rag, api_keys, duplicates, match
        ...
        async with async_session() as db_sess:
            ...
            if cls_config and getattr(cls_config, "auto_classify_enabled", False):
                ...
                trigger_tags = [
                    ...
                    (getattr(cls_config, "auto_classify_only_tag_ids", None) or [])
                    if tag_id > 0
                ]
                if trigger_tags:
                    _auto_classify_state["enabled"] = True
                    _auto_classify_state["filter_mode"] = "tag"
                    asyncio.get_running_loop().create_task(_auto_classify_loop())
                    logging.getLogger(__name__).info(
                        "Auto-classify auto-started in tag-only mode with %d AND-linked trigger tags",
                        len(trigger_tags),
                    )
                else:
                    logging.getLogger(__name__).warning(
                        "Auto-classify not started: tag-only mode requires at least one trigger tag"
                    )
    except Exception as e:
...
async def health_check():
    return {"status": "healthy", "service": "Paperless Classification"}

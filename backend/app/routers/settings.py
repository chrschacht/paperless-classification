...
CUSTOM_BACKGROUND_FILE = Path("/app/data/custom-background.jpg")
MAX_BACKGROUND_FILE_SIZE = 25 * 1024 * 1024
MAX_BACKGROUND_PIXELS = 60_000_000
...
async def get_llm_providers(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(LLMProvider)
        .where(LLMProvider.name.in_(["ollama", "openai"]))
...
async def update_llm_provider(
    ...
    if provider.name not in {"ollama", "openai"} or data.name not in {"ollama", "openai"}:
        raise HTTPException(status_code=400, detail="Nur Ollama und OpenAI werden unterstützt")
...
async def get_app_settings(db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "custom_background": CUSTOM_BACKGROUND_FILE.exists(),
        "background_version": int(CUSTOM_BACKGROUND_FILE.stat().st_mtime_ns) if CUSTOM_BACKGROUND_FILE.exists() else 0,
    }


@router.get("/app/background-image")
async def get_custom_background_image():
    """Return the user-provided background image without additional processing."""
    if not CUSTOM_BACKGROUND_FILE.exists():
        raise HTTPException(status_code=404, detail="Kein eigenes Hintergrundbild gespeichert")
    return FileResponse(
...
@router.post("/app/background-image")
async def upload_custom_background_image(file: UploadFile = File(...)):
    """Validate and persist a custom background as a browser-friendly JPEG."""
    content = await file.read(MAX_BACKGROUND_FILE_SIZE + 1)
    if not content:
        raise HTTPException(status_code=400, detail="Die Bilddatei ist leer")
    if len(content) > MAX_BACKGROUND_FILE_SIZE:
        raise HTTPException(status_code=413, detail="Das Hintergrundbild darf maximal 25 MB groß sein")

    try:
        with Image.open(io.BytesIO(content)) as source:
            source.verify()
        with Image.open(io.BytesIO(content)) as source:
            if source.width * source.height > MAX_BACKGROUND_PIXELS:
                raise HTTPException(
                    status_code=413,
                    detail="Das Hintergrundbild darf maximal 60 Megapixel groß sein",
                )
            ...
            CUSTOM_BACKGROUND_FILE.parent.mkdir(parents=True, exist_ok=True)
            temporary_file = CUSTOM_BACKGROUND_FILE.with_suffix(".tmp.jpg")
            normalized.save(temporary_file, "JPEG", quality=91, optimize=True, progressive=True)
        ...
        raise HTTPException(
            status_code=400,
            detail="Ungültiges Bild. Unterstützt werden JPEG, PNG und WebP.",
        )
    finally:
        await file.close()
    ...
    return {
        ...
        "background_version": int(CUSTOM_BACKGROUND_FILE.stat().st_mtime_ns),
    }


@router.delete("/app/background-image")
async def remove_custom_background_image():
    """Restore the bundled default background image."""
    removed = CUSTOM_BACKGROUND_FILE.exists()
    if removed:
...
async def update_app_settings(
    ...
    if data.classifier_provider is not None:
        if data.classifier_provider not in {"ollama", "openai"}:
            raise HTTPException(status_code=400, detail="Nur Ollama und OpenAI werden unterstützt")

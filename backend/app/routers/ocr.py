...
from app.services.ocr_service import OcrService, batch_state, watchdog_state, single_ocr_running, ocr_page_progress, load_review_queue, save_review_queue, load_ocr_ignore_list, save_ocr_ignore_list, load_ocr_error_list, save_ocr_error_list, load_ocr_error_counts, save_ocr_error_counts, DEFAULT_OLLAMA_URL, DEFAULT_OCR_MODEL, DEFAULT_MAX_FILE_SIZE_MB
    ...
    if SETTINGS_FILE.exists():
        try:
            with open(SETTINGS_FILE, "r") as f:
                ...
                settings.setdefault("max_file_size_mb", DEFAULT_MAX_FILE_SIZE_MB)
...
class OcrSettingsRequest(BaseModel):
    ...
    max_file_size_mb: int = Field(default=DEFAULT_MAX_FILE_SIZE_MB, ge=1, le=500)
...
class BatchOcrRequest(BaseModel):
    # Retained in the payload for compatibility. Only tagged processing is valid.
    mode: str = "tagged"
...
def get_ocr_service() -> OcrService:
    ...
    return OcrService(
        ...
        max_file_size_mb=ocr_settings.get("max_file_size_mb", DEFAULT_MAX_FILE_SIZE_MB),
...
async def save_ocr_settings_endpoint(request: OcrSettingsRequest, client: PaperlessClient = Depends(get_paperless_client)):
    ...
    ocr_settings["max_file_size_mb"] = request.max_file_size_mb
    ...
    # Remove only size-related ignore entries that now fit under the configured
    # limit. Older entries have the size only in their human-readable reason.
    ignore_list = load_ocr_ignore_list()
    filtered_ignore_list = []
    for entry in ignore_list:
        reason = str(entry.get("reason", ""))
        file_size_mb = entry.get("file_size_mb")
        if file_size_mb is None and reason.startswith("Dokument zu groß für OCR"):
            match = re.search(r"\(([0-9]+(?:\.[0-9]+)?) MB >", reason)
            file_size_mb = float(match.group(1)) if match else None
        if file_size_mb is not None and float(file_size_mb) <= request.max_file_size_mb:
            continue
        filtered_ignore_list.append(entry)
    if len(filtered_ignore_list) != len(ignore_list):
        save_ocr_ignore_list(filtered_ignore_list)
...
async def clear_ocr_history(db: AsyncSession = Depends(get_db)):
    """Clear local OCR history without changing documents or tags in Paperless."""
    if (
        ocr_service_module.batch_state["running"]
        or ocr_service_module.watchdog_state["running"]
        or ocr_service_module.single_ocr_running
    ):
        raise HTTPException(
            status_code=409,
            detail="Der OCR-Verlauf kann nur zurückgesetzt werden, wenn keine OCR-Verarbeitung aktiv ist.",
        )

    page_result = await db.execute(delete(OcrPageResult))
    await db.commit()

    review_count = len(load_review_queue())
    ignore_count = len(load_ocr_ignore_list())
    error_count = len(load_ocr_error_list())
    pending_error_count = len(load_ocr_error_counts())
    stats_count = len(get_ocr_service().get_stats())

    save_review_queue([])
    save_ocr_ignore_list([])
    save_ocr_error_list([])
    save_ocr_error_counts({})
    stats_file = ocr_service_module.OCR_STATS_FILE
    stats_file.parent.mkdir(parents=True, exist_ok=True)
    stats_file.write_text("[]")
    ocr_service_module.ocr_page_progress.clear()
    ocr_service_module.batch_state.update({
        "should_stop": False,
        "total": 0,
        "processed": 0,
    ...
    page_result_count = max(page_result.rowcount or 0, 0)
    logger.warning(
        "OCR history reset: %s page rows, %s stats, %s review, %s ignored, %s errors, %s pending errors removed",
        page_result_count,
        stats_count,
        review_count,
        ignore_count,
...
async def ocr_single_document(
    ...
    """Run OCR on a single document using Ollama Vision."""
    try:
...
async def start_batch_ocr(
    ...
    if request.mode != "tagged":
        raise HTTPException(
            status_code=400,
            detail="Paperless Classification verarbeitet OCR-Batches ausschließlich mit dem Tag 'runocr'.",
        )
...
async def evaluate_ocr_results(
    ...
    WARNING: This sends document text to OpenAI.
    ...
    # OpenAI is the only supported cloud evaluator.
    ...
    if not provider:
        raise HTTPException(
            status_code=400,
            detail="OpenAI ist nicht konfiguriert. Bitte unter Einstellungen → LLM einen API-Key hinterlegen."
        )
...
{{
  "ranking": [
    {{
      ...
      "verdict": "<1-2 Sätze Produktionsurteil auf Deutsch>"
    }}
  ],

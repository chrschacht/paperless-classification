...
def effective_custom_field_document_types(
    field_name: str, configured_types: Optional[List[str]],
) -> List[str]:
    """Return document types after applying non-configurable domain rules."""
    effective_types = [
        str(document_type).strip()
        for document_type in (configured_types or [])
        if str(document_type).strip()
    ]
    if str(field_name or "").strip().casefold() == "steuernummer":
        effective_types = [
            document_type for document_type in effective_types
            if document_type.casefold() != "eingangsrechnung"
        ]
...
class ToolExecutor:
    ...
    async def _get_custom_field_definitions(self, _args: Dict) -> List[Dict]:
        ...
        for cfm in self.custom_field_mappings:
            if not cfm.enabled:
                continue
            applicable_document_types = effective_custom_field_document_types(
                cfm.paperless_field_name, cfm.applicable_document_types,
            )
            entry: Dict[str, Any] = {

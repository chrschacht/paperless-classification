...
- Kassenbon/Quittung ohne eindeutig bezeichnete Bon- oder Belegnummer: "Kassenbon <Datum> - <Korrespondent>" beziehungsweise "Bonduplikat <Datum> - <Korrespondent>". Niemals "ohne Rechnungsnummer" in den Titel schreiben.
...
- Waehle 0 bis 5 exakte Namen ausschliesslich aus VERFUEGBARE TAGS. Erfinde keine Tags und keine Schreibvarianten.
- Eine leere Liste [] ist ein korrektes Ergebnis, wenn kein fachlicher Tag eindeutig passt.
- Bewerte den Hauptgegenstand des Gesamtdokuments. Einzelne Leistungspositionen, Anlagen oder Buchungen erzeugen keinen Tag.
- Technische Prozess-Tags wie KI-, OCR-, DATEV-, INBOX und Bezahlen werden ausserhalb der KI gesetzt und duerfen nicht gewaehlt werden.
- Firmennamen, Personennamen und Dokumenttyp-Woerter sind keine fachlichen Tags.
- Mahnung nur bei ausdruecklicher Mahnung oder Zahlungserinnerung.
- Storno nur bei ausdruecklicher Stornorechnung oder einem Stornobeleg.
- Branchen-, Kostenarten- oder Beschaffungs-Tags nur waehlen, wenn der Hauptgegenstand des Gesamtdokuments exakt dazu passt.
- Miete nur fuer Miete, Pacht oder eindeutig zugehoerige Mietnebenkosten.
- Fortbildung nur fuer Fortbildung, Kurs, Kongress oder eindeutig fachbezogene Weiterbildung.
- Personal nur fuer Personalunterlagen; bei lohnsteuerlichem Hauptgegenstand zusaetzlich Steuern.
- Steuern nur bei Finanzamt oder eindeutig steuerlichem Hauptgegenstand.
- Versicherungen, Verbaende, Mitgliedsbeitraege und sonstige Kosten erhalten keinen Ersatz-Tag, wenn dafuer kein exakter Tag verfuegbar ist.
...
SYSTEM_PROMPT_OPENAI = f"""Fuehre eine praezise Dokumentenklassifikation fuer ein Paperless-ngx Dokumentenmanagementsystem durch.
...
SYSTEM_PROMPT_OLLAMA_ANALYZE = f"""Fuehre eine Dokumentenklassifikation durch. Extrahiere Informationen als JSON.

WICHTIGSTE REGEL -- LIES DEN TEXT GENAU:
- Die ersten Zeilen koennen die Dokumentbezeichnung enthalten. Pruefe sie gegen das Gesamtdokument.
- Uebernimm die tatsaechliche Bezeichnung: "Abrechnung der Brutto/Netto-Bezuege" ist eine
  Entgeltabrechnung und KEINE Lohnsteuerbescheinigung.
- Lohnjournal, Entgeltabrechnung, Lohnsteuerbescheinigung und SV-Meldung sind verschiedene Dokumente.
...
- Bei Kassenbons: Beleg- oder Bonnummer bevorzugen; falls nicht vorhanden, eine ausdruecklich bezeichnete Transaktions- oder TSE-Vorgangsnummer verwenden.
- Keine Kennung ergaenzen, kuerzen, normalisieren oder mit einem vermuteten Praefix versehen

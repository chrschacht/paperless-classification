...
_SCHEMA_CORRESPONDENT = {
    "type": "object",
    "required": ["correspondent"],
    "properties": {"correspondent": {"type": ["string", "null"]}},
    "additionalProperties": False,
}
...
class OllamaMultiCallProvider(BaseClassifierProvider):
    ...
    async def classify(
        ...
        try:
            ...
            correspondent_catalog = ""
            if config.get("enable_correspondent", True) and self.tool_executor:
                try:
                    existing_correspondents = await self.tool_executor.paperless.get_correspondents(use_cache=True)
                    names = [c.get("name", "") for c in existing_correspondents if c.get("name")]
                    if names:
                        correspondent_catalog = (
                            "\n\nVORHANDENE KORRESPONDENTEN (bei passendem Aussteller Namen exakt übernehmen):\n"
                            + " | ".join(names[:500])
                        )
                except Exception as exc:
                    logger.warning("Could not load correspondent catalog for Ollama prompt: %s", exc)
            ...
            analyze_user_msg = (
                f"Aktueller Titel: {document.current_title}{existing_block}{correspondent_catalog}"
            ...
            if config.get("enable_document_type", True) and self.tool_executor:
                ...
                if type_names:
                    dtype_rules = config.get("prompt_document_type") or RULES_DOCTYPE
                    dtype_prompt = (
            ...
            # Call 1 happens before the document type is known. On outgoing
            # invoices a local model can therefore mistake the prominent letterhead
            # for the correspondent. Once the direction is known, resolve the
            # external recipient in a dedicated, practice-independent pass.
            if result.document_type == "Ausgangsrechnung":
                recipient_prompt = (
                    "Das Gesamtdokument ist eine vom eigenen Unternehmen oder Absender ausgestellte Rechnung. "
                    "Bestimme ausschließlich den externen Rechnungsempfänger beziehungsweise "
                    "Kunden aus dem Empfänger-Adressfeld. Der Rechnungsaussteller, der eigene "
                    "Absender und Namen in Leistungszeilen sind ausgeschlossen. "
                    "Bei natürlichen Personen gib Vor- und Nachnamen ohne Anrede, Adresse oder Kundennummer aus. "
                    "Wenn der Empfänger nicht eindeutig lesbar ist, gib null zurück. "
                    'Antworte als JSON: {"correspondent":"Vorname Nachname oder null"}'
                )
                recipient_response = await self._call_ollama(
                    recipient_prompt,
                    f"--- RECHNUNG ---\n{content[:4000]}",
                    max_tokens=100,
                    json_schema=_SCHEMA_CORRESPONDENT,
                )
                total_calls += 1
                recipient_data = self._parse_json(recipient_response)
                recipient = recipient_data.get("correspondent") if isinstance(recipient_data, dict) else None
                if recipient and not self._is_hallucinated_correspondent(str(recipient)):
                    result.correspondent = str(recipient).strip()
                else:
                    result.correspondent = None
                result.debug_info["outgoing_recipient_raw_response"] = recipient_response[:300]
            ...
            if config.get("enable_tags", True) and self.tool_executor:
                ...
                if all_tags:
                    tags_min = config.get("tags_min", 0)
                    ...
                    for t in raw_tags:
                        ...
                        if t in candidate_tags:
                            ...
                            if config.get("tag_behavior", "existing_only") == "existing_only":
                                logger.info(
                                    "Discarding unknown tag '%s' because tag_behavior=existing_only",
                                    t,
                                )
                                continue
            ...
            if config.get("enable_custom_fields", False) and self.tool_executor:
                ...
                if fields:
                    selected_type = (result.document_type or "").casefold()
                    fields = [
                        field for field in fields
                        if selected_type in {
                            name.casefold() for name in (field.get("applicable_document_types") or [])
                        }
                    ]

                if fields:
                    allowed_field_names = {field["field_name"].casefold(): field["field_name"] for field in fields}
                    fields_text = "\n".join(
                    ...
                    if isinstance(cf_data, dict):
                        # Local models sometimes invent plausible extra keys even when
                        # only one field was offered. Never expose or apply those keys.
                        aliases = {
                            "gesamtbetrag": "betrag",
                            "bruttobetrag": "betrag",
                            "rechnungsnr": "rechnungsnummer",
                            "rechnungs-nr": "rechnungsnummer",
                            "auszug-nr": "auszugsnummer",
                        }
                        filtered_fields = {}
                        for name, value in cf_data.items():
                            if not isinstance(name, str):
                                continue
                            normalized_name = name.strip().casefold().rstrip(".:")
                            normalized_name = aliases.get(normalized_name, normalized_name)
                            if normalized_name in allowed_field_names:
                                filtered_fields[allowed_field_names[normalized_name]] = value
                        result.custom_fields = filtered_fields
                        result.debug_info["custom_fields_raw_response"] = cf_response[:500]

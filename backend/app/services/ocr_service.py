...
TAG_FORCE_OCR = "forceocr"
TAG_CLASSIFY = "KI-klassifizieren"
...
# OCR run statistics shown in the frontend
OCR_STATS_FILE = Path("/app/data/ocr_stats.json")
...
class OcrService:
    ...
    def _extract_text_from_pdf(self, file_bytes: bytes) -> Optional[str]:
        ...
        Smart-Skip prefers a plausible existing text layer, regardless of
        whether it came from a digital source or from Paperless/OCRmyPDF.  PDF
        metadata is logged for transparency but is no longer a rejection
        criterion by itself.  Vision OCR is only forced when the extracted text
        fails conservative quality checks.
        """
        ...
        try:
            ...
            # Metadata is useful for diagnostics, but Paperless/OCRmyPDF and
            # Tesseract commonly produce perfectly usable text layers.
            meta = reader.metadata or {}
            creator = (meta.get("/Creator", "") or "").lower()
            producer = (meta.get("/Producer", "") or "").lower()

            # Extract every page.  The returned value is later written back to
            # Paperless, so limiting detection to the first pages would
            # truncate longer documents when OCR is skipped.
            page_texts = [(page.extract_text() or "").strip() for page in reader.pages]
            text = "\n\n".join(part for part in page_texts if part).strip()
            accepted, reason = assess_pdf_text_quality(text, len(reader.pages))
            source = f"creator={creator or '-'}, producer={producer or '-'}"
            if not accepted:
                logger.info("Smart-Skip rejected PDF text (%s; %s). Forcing OCR.", reason, source)
                return None

            logger.info("Smart-Skip accepted PDF text (%s; %s).", reason, source)
            return text
        except Exception as e:
    ...
    async def ocr_document(self, paperless_client, document_id: int, force: bool = False, db_session=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_mb > self.max_file_size_mb:
                ocr_page_progress.pop(document_id, None)
                error_msg = (
                    f"Dokument zu groß für OCR ({file_mb:.1f} MB > {self.max_file_size_mb} MB). "
    ...
    async def batch_ocr(
        ...
        """Run a tag-triggered OCR batch and update batch_state in-place."""
        global batch_state

        if mode != "tagged":
            raise ValueError(
                "Paperless Classification verarbeitet OCR-Batches ausschließlich mit dem Tag 'runocr'."
            )
        ...
        try:
            ...
            forceocr_tag = await paperless_client.get_or_create_tag(TAG_FORCE_OCR)
            forceocr_tag_id = forceocr_tag.get("id")
            classify_tag = await paperless_client.get_or_create_tag(TAG_CLASSIFY)
            classify_tag_id = classify_tag.get("id")
            ...
            # The runocr tag is the sole source of truth.  Never enumerate all
            # Paperless documents or infer work from local database contents.
            if runocr_tag_id:
                documents = await paperless_client.get_documents(tag_id=runocr_tag_id)
                documents = [
                    document for document in documents
                    if ocrfinish_tag_id not in document.get("tags", [])
                    and ocrreview_tag_id not in document.get("tags", [])
                    and ocrerror_tag_id not in document.get("tags", [])
                ]
            normal_count = len(documents)
            force_documents = []
            if forceocr_tag_id:
                force_documents = await paperless_client.get_documents(tag_id=forceocr_tag_id)
                for document in force_documents:
                    document["_force_ocr"] = True
            documents_by_id = {document.get("id"): document for document in documents}
            for document in force_documents:
                documents_by_id[document.get("id")] = document
            documents = [doc for doc_id, doc in documents_by_id.items() if doc_id is not None]
            batch_state["log"].append(
                f"🏷️ Tag-Modus: {normal_count}× runocr, {len(force_documents)}× forceocr"
            )
            ...
            if ignored_ids:
                ...
                skipped_docs = [
                    ...
                    if d.get("id") in ignored_ids and not d.get("_force_ocr")
                ]
                documents = [
                    d for d in documents
                    if d.get("id") not in ignored_ids or d.get("_force_ocr")
                ]
            ...
            for i, doc in enumerate(documents):
                ...
                is_force_ocr = bool(doc.get("_force_ocr"))
                ...
                try:
                    # OCR is provided exclusively by Ollama Vision.
                    from app.database import async_session
                    async with async_session() as db_sess:
                        ocr_result = await self._ocr_document_via_provider(
                            paperless_client, doc_id, force=is_force_ocr,
                            db_session=db_sess,
                        )
                    ...
                    if new_content:
                        ...
                        needs_review = False
                        if is_force_ocr:
                            quality_ok, quality_reason = assess_pdf_text_quality(
                                new_content, ocr_pages,
                            )
                            if not quality_ok:
                                needs_review = True
                                batch_state["log"].append(
                                    f"⚠️ {doc_title}: Force-OCR unplausibel ({quality_reason}) → In Prüfliste"
                                )
                                ...
                                queue.append({
                                    ...
                                    "old_length": old_len,
                                    "new_length": new_len,
                                    "ratio": round(new_len / old_len * 100) if old_len else 0,
                                    "suggest_keep_original": False,
                                    "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
                                    "retried": True,
                                })
                                save_review_queue(queue)
                                add_t = [ocrreview_tag_id] if ocrreview_tag_id else []
                                rem_t = [forceocr_tag_id] if forceocr_tag_id else []
                                try:
                                    await paperless_client.bulk_update_documents(
                                        document_ids=[doc_id],
                                        add_tags=add_t or None,
                                        remove_tags=rem_t or None,
                                    )
                                except Exception as tag_err:
                                    logger.error("Failed to set OCR review tag for %s: %s", doc_id, tag_err)
                        elif old_len > 100 and new_len < old_len * QUALITY_THRESHOLD:
                        ...
                        if not needs_review:
                            ...
                            if is_force_ocr and classify_tag_id:
                                add_tags.append(classify_tag_id)
                            if remove_runocr_tag and runocr_tag_id:
                                remove_tags.append(runocr_tag_id)
                            if is_force_ocr and forceocr_tag_id:
                                remove_tags.append(forceocr_tag_id)
                            ...
                            if not tag_success:
                                ...
                                if is_force_ocr:
                                    batch_state["log"].append(
                                        f"✅ {doc_title}: Force-OCR erfolgreich; Klassifizierung erneut eingeplant ({new_len} Zeichen)"
                                    )
                                else:
    ...
    async def watchdog_loop(self, paperless_client):
        ...
        async def _get_watchdog_tag_ids():
            nonlocal _runocr_tag, _ocrfinish_tag, _ocrpruefen_tag, _ocrerror_tag, _forceocr_tag
            try:
                if _runocr_tag is None:
                    _runocr_tag = await paperless_client.get_or_create_tag(TAG_RUN_OCR)
                ...
                if _forceocr_tag is None:
                    _forceocr_tag = await paperless_client.get_or_create_tag(TAG_FORCE_OCR)
                excluded = [
                    t["id"] for t in [_ocrfinish_tag, _ocrpruefen_tag, _ocrerror_tag]
                    if t and t.get("id")
                ]
                return (
                    _runocr_tag.get("id") if _runocr_tag else None,
                    _forceocr_tag.get("id") if _forceocr_tag else None,
                    excluded,
                )
            except Exception:
                return None, None, []
            ...
            try:
                ...
                if batch_state["running"] or single_ocr_running or ollama_is_locked():
                    ...
                    if not conn.get("connected"):
                        ...
                        try:
                            runocr_id, forceocr_id, exclude_ids = await _get_watchdog_tag_ids()
                            if runocr_id or forceocr_id:
                                force_count = 0
                                if forceocr_id:
                                    force_count = await paperless_client.get_document_count(
                                        tags_id_all=[forceocr_id]
                                    )
                                pending_count = 0
                                if runocr_id:
                                    pending_count = await paperless_client.get_document_count(
                                        tags_id_all=[runocr_id],
                                        tags_id_none=exclude_ids
                                    )
                                if pending_count == 0 and force_count == 0:
                                    logger.info(
                                        "Watchdog: Keine Dokumente mit 'runocr' oder 'forceocr' – überspringe diesen Zyklus"
                                    )
                                    should_run = False
                                else:
                                    logger.info(
                                        "Watchdog: %d× runocr, %d× forceocr; starte Tag-Batch...",
                                        pending_count, force_count,
                                    )
                            else:
                                logger.warning("Watchdog: OCR-Tags nicht verfügbar – Zyklus übersprungen")
                                should_run = False
                        except Exception as check_err:
                            logger.warning(
                                "Watchdog: Tag-Pre-Check fehlgeschlagen; aus Sicherheitsgründen kein Batch: %s",
                                check_err,
                            )
                            should_run = False

"""Models for the KI-Klassifikation feature."""
...
class ClassifierConfig(Base):
    ...
    tags_min = Column(Integer, default=0)
    ...
    # A document must contain ALL configured trigger tags.
    ...
    # Additional deterministic status tags selected by document type.
    # Each rule: {id, enabled, tag_id, tag_name, document_types[],
    #             skip_if_tag_id?, skip_if_tag_name?}.
    status_tag_rules = Column(JSON, default=[])
...
class CustomFieldMapping(Base):
    ...
    # Extraction and apply are limited to these exact Paperless document type names.
    # Empty means no document type (the field can also be disabled entirely).
    applicable_document_types = Column(JSON, default=[])
    # Subset of applicable_document_types whose value is required before an
    # automatic classification may be applied.
    required_document_types = Column(JSON, default=[])
...
class ClassifierOcrRetryState(Base):
    """Persistent marker that a missing-field Force-OCR was already requested."""
    __tablename__ = "classifier_ocr_retry_state"

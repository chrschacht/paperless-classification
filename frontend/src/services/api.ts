...
export const getBackgroundImageUrl = (version: number = 0) =>
  `${API_BASE}/settings/app/background-image${version ? `?v=${version}` : ''}`

export const uploadBackgroundImage = async (file: File) => {
  const formData = new FormData()
  formData.append('file', file)
  const response = await fetch(`${API_BASE}/settings/app/background-image`, {
    method: 'POST',
    body: formData,
  })
  if (!response.ok) {
...
export const removeBackgroundImage = () =>
  fetchJson<{ success: boolean; removed: boolean }>('/settings/app/background-image', {
    method: 'DELETE',
  })
...
export const getOcrSettings = () =>
  fetchJson<{ ollama_url: string; ollama_urls: string[]; model: string; max_image_size: number; max_file_size_mb: number; smart_skip_enabled: boolean; watchdog_enabled?: boolean; watchdog_interval?: number }>('/ocr/settings')

export const saveOcrSettings = (data: { ollama_url: string; ollama_urls?: string[]; model: string; max_image_size: number; max_file_size_mb: number; smart_skip_enabled: boolean }) =>
...
export interface OcrHistoryResetResult {
  cleared: boolean
...
export const resetOcrHistory = () =>
  fetchJson<OcrHistoryResetResult>('/ocr/history', { method: 'DELETE' })
...
// Batch OCR
export const startBatchOcr = (mode: 'tagged' = 'tagged', documentIds?: number[], setFinishTag: boolean = true, removeRunocrTag: boolean = true) =>
...
export interface ClassifierConfig {
  ...
  status_tag_rules: StatusTagRule[]
}

export interface StatusTagRule {
  id: string
  enabled: boolean
  tag_id?: number | null
  tag_name: string
  document_types: string[]
  skip_if_tag_id?: number | null
  skip_if_tag_name?: string
}
...
export interface ClassificationResult {
  ...
  missing_required_custom_fields?: string[]
  tokens_input: number
...
export interface PortableConfigurationImportResult {
  status: string
  auto_classify_enabled: boolean
  status_rules_saved: number
  storage_profiles_saved: number
  custom_field_mappings_saved: number
  warnings: Record<string, string[]>
}

export const exportClassifierConfiguration = () =>
  fetchJson<Record<string, any>>('/classifier/configuration/export')

export const importClassifierConfiguration = (data: Record<string, any>) =>
  fetchJson<PortableConfigurationImportResult>('/classifier/configuration/import', {
    method: 'POST',
    body: JSON.stringify(data),
  })
...
export const resetClassificationHistory = () =>
  fetchJson<{ cleared: boolean; history_count: number; ocr_retry_count: number }>(
    '/classifier/history',
    { method: 'DELETE' },
  )

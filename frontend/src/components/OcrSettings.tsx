...
export default function OcrSettings() {
    ...
    const loadSettings = async () => {
        try {
            ...
            setMaxFileSizeMb(settings.max_file_size_mb || 25)
    ...
    const resetHistory = async () => {
        const confirmed = window.confirm(
            'Gesamten OCR-Verlauf unwiderruflich zurücksetzen?\n\n' +
            'Gelöscht werden lokale Statistiken, Seiten-Zwischenergebnisse, Prüfwarteschlange, Fehlerlisten und die OCR-Ausnahmeliste. ' +
            'Dokumenttexte und OCR-Tags in Paperless bleiben unverändert. Der Vorgang ist nur möglich, wenn keine OCR-Verarbeitung läuft.'
        )
        if (!confirmed) return
        ...
        try {
            const result = await api.resetOcrHistory()
            const removed = result.page_result_count + result.stats_count + result.review_count + result.ignore_count + result.error_count + result.pending_error_count
            setHistoryResetStatus(`${removed} lokale OCR-Verlaufseinträge wurden gelöscht.`)
        } catch (e: any) {
    ...
    return (
        <div className="card p-0 overflow-hidden border border-surface-700/50 shadow-xl shadow-black/20 bg-surface-800/40 backdrop-blur-sm">
            ...
            <div className="p-6 space-y-8">
                {/* Ollama is the sole OCR provider. */}
                <div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                step="1"
                                value={maxFileSizeMb}
                                onChange={(e) => setMaxFileSizeMb(Math.min(500, Math.max(1, parseInt(e.target.value) || 25)))}
                                className="w-full input bg-surface-900/50 border-surface-700 focus:border-blue-500 font-mono text-sm pr-12"
                            />
                            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-surface-500">MB</span>
                        </div>
                        <p className="text-xs text-surface-500 mt-1">
                            Größere Originaldateien werden nicht in Seitenbilder umgewandelt. Empfohlen: 25 MB.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="bg-surface-900/30 rounded-xl p-3 border border-surface-700/50">
                            <div className="flex items-center justify-between">
                                <div>
                                    <span className="text-sm font-medium text-surface-200 block">Smart-Skip (Qualitätsprüfung)</span>
                                    <p className="text-xs text-surface-500">Übernimmt plausiblen PDF-Text und startet OCR nur bei Qualitätsproblemen</p>
                                </div>
                ...
                <div className="overflow-hidden rounded-3xl border border-red-300/60 bg-gradient-to-r from-red-50/80 via-white/45 to-amber-50/55 shadow-lg shadow-red-900/5">
                    <div className="flex flex-col gap-5 p-5 lg:flex-row lg:items-center lg:justify-between">
                        <div className="max-w-3xl">
                            <div className="mb-2 flex items-center gap-2 text-red-700">
                                <Trash2 className="h-5 w-5" />
                                <h3 className="text-lg font-bold">OCR-Verlauf zurücksetzen</h3>
                            </div>
                            <p className="text-sm leading-6 text-surface-600">
                                Löscht lokale Statistiken, Seiten-Zwischenergebnisse, Prüfwarteschlange, Fehlerlisten und
                                die OCR-Ausnahmeliste. Dokumenttexte und OCR-Tags in Paperless bleiben unverändert.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    {historyResetStatus && (
                        <p className="mx-5 mb-5 rounded-xl border border-white/70 bg-white/55 px-4 py-3 text-sm text-surface-700">
                            {historyResetStatus}
                        </p>
                    )}
                </div>

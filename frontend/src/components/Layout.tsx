...
const baseNavigation: NavItem[] = [
  ...
  { name: 'KI-Klassifikation', href: '/classifier', icon: Sparkles, alwaysShow: true },
...
const applyBackgroundImage = (settings: api.AppSettingsResponse) => {
...
export default function Layout({ children }: LayoutProps) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      const activeNavItem = document.querySelector<HTMLElement>('[data-active-nav="true"]')
  ...
  useEffect(() => {
    ...
    api.getAppSettings()
      .then(appSettings => {
        applyBackgroundImage(appSettings)
  ...
  return (
    <div className="min-h-screen flex relative isolate">
      ...
      <aside className={clsx(
                        ...
                        return (
                          <Link
                            ...
                            className={clsx(
                              ...
                              isActive
                                ? 'bg-white/15 text-white border border-white/35 shadow-sm'
                                : 'text-white/80 hover:text-white hover:bg-white/10'
                            )}
                          >
        ...
        {/* Footer */}
        <div className="shrink-0 p-4 border-t border-surface-700/50 space-y-2">
          <div className="text-xs text-surface-600 text-center">
            Paperless Classification v{packageInfo.version}
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        ...
        {/* Page content */}
        <main className="flex-1 px-4 py-6 lg:px-7 overflow-auto">

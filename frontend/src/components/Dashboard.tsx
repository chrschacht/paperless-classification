  ...
  return (
    <div className="relative select-none">
      <svg width={200} height={200} viewBox="0 0 200 200">
        {/* Track */}
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="#cbd5e1" strokeWidth={24} />
        ...
        {/* Center */}
        <text x={cx} y={cy - 9} textAnchor="middle" fill="#ffffff"
          fontSize="22" fontWeight="700" fontFamily="ui-monospace,monospace">{centerTop}</text>
        <text x={cx} y={cy + 12} textAnchor="middle" fill="rgba(255,255,255,0.82)"
    ...
    <div className="dashboard-page space-y-5 pb-8">
          ...
          {[
            ...
            return (
              <div key={c.label} className="card p-5 border border-surface-700/50 bg-surface-800/50">
                ...
                <div className="font-display text-3xl font-bold text-surface-100 mb-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Link to="/ocr" className="group card p-5 border border-cyan-500/20 bg-gradient-to-b from-cyan-500/5 to-transparent hover:border-cyan-500/40 transition-all hover:scale-[1.01]">
          ...
          <div className="flex items-end gap-2 mb-3">
            <span className="font-display text-4xl font-bold text-cyan-700">{ocrPct}%</span>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 text-center">
            {[
              { val: ocrDone, label: 'Fertig', color: 'text-cyan-700' },
              { val: ocrReview, label: 'Prüfen', color: ocrReview > 0 ? 'text-orange-700' : 'text-surface-400' },
              { val: ocrIgnored, label: 'Ignoriert', color: 'text-surface-400' },
            ].map(i => (
              <div key={i.label} className="glass-stat-tile p-2 rounded-xl">
        ...
        <Link to="/classifier" className="group card p-5 border border-violet-500/20 bg-gradient-to-b from-violet-500/5 to-transparent hover:border-violet-500/40 transition-all hover:scale-[1.01]">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-display font-semibold text-surface-100">KI-Klassifikation</span>
            </div>
            <ArrowRight className="w-4 h-4 text-surface-600 group-hover:text-violet-400 group-hover:translate-x-1 transition-all" />
          </div>

          <div className="flex items-end gap-2 mb-3">
            <span className="font-display text-4xl font-bold text-violet-700">{classifiedPct}%</span>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 text-center">
            {[
              { val: applied, label: 'Angewendet', color: 'text-emerald-700' },
              { val: analyzed, label: 'Offen', color: analyzed > 0 ? 'text-amber-700' : 'text-surface-400' },
              { val: classStats?.total_runs || 0, label: 'Analysen', color: 'text-violet-700' },
            ].map(i => (
              <div key={i.label} className="glass-stat-tile p-2 rounded-xl">
                <div className={clsx('text-lg font-bold', i.color)}>{i.val.toLocaleString('de-DE')}</div>
                <div className="text-xs text-surface-500">{i.label}</div>
        ...
        <Link to="/correspondents" className="group card p-5 border border-amber-500/20 bg-gradient-to-b from-amber-500/5 to-transparent hover:border-amber-500/40 transition-all hover:scale-[1.01]">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4 text-center">
            {[
              { val: counts.correspondents, label: 'Korr.', cleaned: cleanupStats?.correspondents?.merged + cleanupStats?.correspondents?.deleted || 0, color: 'text-blue-700' },
              { val: counts.tags, label: 'Tags', cleaned: cleanupStats?.tags?.merged + cleanupStats?.tags?.deleted || 0, color: 'text-purple-700' },
              { val: counts.documentTypes, label: 'Typen', cleaned: cleanupStats?.document_types?.merged || 0, color: 'text-amber-700' },
            ].map(i => (
              <div key={i.label} className="glass-stat-tile p-2 rounded-xl">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-emerald-700">{totalCleaned}</span>
            </div>
            <div className="flex justify-between text-sm">
          ...
          <div className="lg:col-span-2 card p-6 border border-surface-700/50">
            ...
            <div className="space-y-4">
              {classStats.by_provider?.map((p: any) => {
                ...
                const isCloud = p.provider === 'openai'
                return (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { href: '/classifier', icon: Brain, label: 'Klassifikation', sub: `${(classStats?.total_documents_paperless || 0) - applied} offen`, color: 'border-violet-500/25 hover:border-violet-500/50', ic: 'text-violet-400', bg: 'bg-violet-500/5' },

...
Alle wichtigen Änderungen an Paperless Classification.
...
- **Öffentliche Erstveröffentlichung**: Bereinigter Quellstand mit datenschutzgeprüften Screenshots, neuer Benutzer-, Sicherheits- und Upgrade-Dokumentation sowie unveränderter MIT-Lizenz des Ursprungsprojekts.
- **Herkunft dokumentiert**: Die Weiterentwicklung auf Basis von AI Paperless Organizer wird in README und Drittanbieterhinweisen ausdrücklich genannt.
- **Neuer Projektname**: Anwendung, Frontend, Dokumentation, Compose-Dienste und Container-Images heißen durchgängig `paperless-classification`.
- **Klare Navigation**: Der Menüpunkt heißt jetzt „KI-Klassifikation“.
- **Liquid-Glass-Oberfläche**: Transluzenz, systemnahe Typografie, räumliche Ebenen und klare Kontraste modernisieren die Oberfläche ohne die Bedienabläufe zu verändern.
- **Zwei LLM-Provider**: Die Anwendung unterstützt ausschließlich Ollama und OpenAI; nicht mehr unterstützte Provider werden aus API, Oberfläche, Laufzeit und Datenbank entfernt.
- **Bereinigtes Schema**: Veraltete provider- und klassifikationsbezogene Spalten werden nach sicherer Übernahme noch relevanter Ollama-/OpenAI-Werte entfernt. Aktiv genutzte Fachdaten bleiben erhalten.
- **Fokus auf Paperless-ngx**: Cloud Sync / Import einschließlich rclone, WebDAV, Hintergrunddienst, API und Datenbanktabellen wurde entfernt. Dokumente werden ausschließlich aus Paperless-ngx verarbeitet.
- **Klarere Navigation**: OCR steht direkt nach der KI-Klassifikation; „Prompts“ ist jetzt ein Unterpunkt von „Aufräumen“.
- **Responsive Einstellungen**: Kompakte Einstellungsbereiche stehen auf breiten Bildschirmen nebeneinander; Status-Tags und zusätzliche Status-Tag-Regeln sind getrennte Kacheln.
- **Verläufe zurücksetzen**: Klassifikations- und OCR-Historien lassen sich in den jeweiligen Einstellungen gezielt zurücksetzen, ohne Paperless-Dokumente zu löschen.
- **Individuelle Darstellung**: Eigenes Hintergrundbild, einheitliche Glass-Kacheln und verbesserte Kontraste in Menü, Titelleiste und Dashboard.

---

## 1.1.3 – 2026-08-25

### Pflichtfelder und OCR-Wiederholung
- **Pflichtfelder je Dokumenttyp**: Custom Fields können als Voraussetzung für die automatische Anwendung markiert werden; vorhandene gültige Paperless-Werte werden berücksichtigt.
- **Einmalige Force-OCR**: Fehlen Pflichtfelder, wechselt das Dokument von `ocrfinish` zu `forceocr`. Nach erfolgreicher OCR werden `ocrfinish` und `KI-klassifizieren` wieder gesetzt.
- **Sichere Eskalation**: Fehlen die Werte auch danach, endet die Wiederholungsschleife und das Dokument erhält `KI-prüfen`.
- **Echte Vorschau**: Fehlende Pflichtfelder und die daraus folgenden Bearbeitungs-Tags sind bereits in der manuellen Klassifizierung sichtbar.

### OCR-Oberfläche
- **Besserer Kontrast**: Warnungen, OCR-Texte und Bedienelemente sind auf hellem Untergrund deutlich lesbarer.
- **PDF-Vorschau**: Die eingebettete Vorschau verwendet einen robusteren Fallback mit direkter Öffnung des Dokuments.

## 1.1.2 – 2026-08-25

### OCR Smart-Skip
- **Vorhandene OCR bevorzugen**: Plausibler PDF-Text aus Paperless, OCRmyPDF oder Tesseract wird übernommen, statt allein wegen der Erzeuger-Metadaten erneut durch Vision-OCR verarbeitet zu werden.
- **Qualitätsprüfung**: Fehlender oder zu kurzer Text, unlesbare Sonderzeichen, Modell-Denktext und Wiederholungsschleifen lösen weiterhin eine neue OCR aus.
- **Vollständige Dokumente**: Smart-Skip übernimmt bei mehrseitigen PDFs den Text aller Seiten und nicht nur den Anfang des Dokuments.

---

## 1.1.1 – 2026-08-23

### Custom Fields bei Eingangsrechnungen
- **Steuernummer**: Lieferanten-Steuernummern und -USt-IDs werden bei Eingangsrechnungen weder angefragt noch vorgeschlagen oder angewendet. Auch widersprüchliche Alt-Konfigurationen können diese Fachregel nicht mehr umgehen.
- **Leere Felder**: Nicht belegte Custom Fields wie Mahnstufe, Zahldatum oder Vertragsnummer werden vollständig aus Vorschau und Apply-Payload entfernt, statt mit `null` angezeigt zu werden.
- **Prompts**: OpenAI-/Mistral- und Ollama-Regeln verlangen nur noch tatsächlich belegte, für den Dokumenttyp relevante Custom Fields.

---

## 1.1.0 – 2026-08-23

### Eigenständiger Paperless Classification
- **Projekttrennung**: Verweise, Menüpunkte und technische Verbindungen zum bisherigen AI Paperless Organizer wurden entfernt; der übliche Lizenzhinweis bleibt erhalten.
- **Eigene Distribution**: Repository, Multi-Arch-Container-Workflow und GHCR-Compose-Datei verwenden den eigenständigen Namen `paperless-classification`.

### Klassifizierung und Buchhaltung
- **Praxisregeln**: Eingangsrechnungen, Beitragsforderungen, Bescheide, Kassenbons und Zahlungsbelege werden buchhalterisch klarer unterschieden.
- **Vorschau entspricht Anwendung**: Status- und Bearbeitungs-Tags werden bereits in der manuellen Vorschau so dargestellt, wie sie später angewendet werden; OCR-Tags bleiben unberührt.
- **Kassenbons**: Bon-/Belegkennungen werden als Rechnungsnummer übernommen, wenn sie tatsächlich vorhanden sind; andernfalls bleibt das Feld leer und der Titel erhält keinen Platzhalter.

### OCR
- **Konfigurierbare Dateigröße**: Die bisher feste 12-MB-Grenze ist als OCR-Einstellung von 1 bis 500 MB konfigurierbar; Standard sind 25 MB.
- **Automatische Freigabe**: Größenbedingt ignorierte Dokumente werden nach einer ausreichenden Grenzerhöhung wieder für OCR zugelassen.
...
- **Feature**: Container-Images aktualisiert
...
- Klassifikationsstatus nur gelb, wenn aktiv klassifiziert wird

### Dokumentation
- README komplett überarbeitet: Feature-Tabelle direkt oben, alle neuen Features dokumentiert
- Neue Screenshots: Dashboard, Klassifikation, RAG Chat, Cloud Import, Duplikate
...
### Grundfunktionen
- KI-Dokumentklassifikation (Titel, Tags, Korrespondent, Typ, Datum, Speicherpfad, Custom Fields)
...
- LLM-Anbindung für OpenAI und Ollama
